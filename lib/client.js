	...
	factory: (require) => {
		...
		function setFaviconBadge(show) {
			if (show) {
				...
				var svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><circle cx="50" cy="14" r="12" fill="#e5484d"/></svg>';
		...
		function beep() {
			try {
				...
				var o = beepCtx.createOscillator();
				var g = beepCtx.createGain();
				o.connect(g);
		...
		function Page(scope, requestNotifyPermission, t, locale) {
			return react.createElement(function NotificationsSettingsPage(_props) {
				...
				var tr = function (key, fallback) { return t ? t(key) : fallback; };
				...
				for (var i = 0; i < rows.length; i++) {
					var r = rows[i];
					var field = r[0];
					var label = tr(field, zhDict[field] || field);
					var checked = !!value[field];
					var onChange = (function (f, cur) {
						return function () {
		...
		function apply(ctx) {
			...
			var seen = new Map(); // sessionId -> { running, pendingKinds, error }
			var notifiedAt = new Map(); // kind -> last fire timestamp
			var disposers = [];
			var watched = new Set();
			...
			function cooldownOk(kind) {
				var c = cfg();
				var cooldownMs = (c.cooldownSec == null ? 30 : c.cooldownSec) * 1000;
				var now = Date.now();
				var last = notifiedAt.get(kind);
			...
			function sessionTitle(id) {
				try {
					...
					var row = list && list.byId ? list.byId[id] : undefined;
			...
			function notify(kind, title, body) {
				...
				var unfocused = typeof document.hasFocus === "function" ? !document.hasFocus() : document.hidden;
				if (!unfocused) return;
				...
				if (c.browserNotify && typeof Notification !== "undefined") {
					if (Notification.permission === "granted") {
						try {
							var n = new Notification(title, { body: body, tag: "dsh-notify-" + kind });
							n.onclick = function () {
			...
			function pendingKinds(snap) {
				...
				if (pend && pend.length) {
					for (var i = 0; i < pend.length; i++) out.push(pend[i].kind);
				}
			...
			function evaluate(sessionId, snap) {
				...
				var prev = seen.get(sessionId);
				seen.set(sessionId, cur);
				if (!prev) return; // 首次见到只建立基线，不打扰

				var label = sessionTitle(sessionId);
				var c = cfg();
			...
			function watchSession(id) {
				...
				watched.add(id);
				disposers.push(
					binding.session.subscribe(function () {
			...
			function syncWatchers() {
				try {
					...
					for (var i = 0; i < ids.length; i++) watchSession(ids[i]);
				} catch (e) { /* ignore */ }
			}
			...
			// 设置页
			if (slots) {
				var injectDisposer = slots.inject("settings.section", function () {
					var regDisposer = slots.register(
						{
							name: "settings.section",
							id: "notifications",
							order: 60,
							label: function () { return tr("nav", "通知"); },
							locale: LOCALE_NS,
						},
						function () { return Page(scope, requestNotifyPermission, t, locale); }
					);
